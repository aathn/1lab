...
bicategory $\bicat{C}$, the morphisms between two objects form a
category instead of a set, so in this setting we get a $\Cat$-valued
pseudofunctor.

Here, we define the covariant mapping $\hom(X,-)$ for a fixed object $X$
in $\bicat{C}$.  The action of this pseudofunctor on 1-cells must take a
1-cell $f : A \to B$ in $\bicat{C}$ to a functor $\hom(X,f) : \hom(X,A)
\to \hom(X,B)$, and this action itself should be functorial.  In other
words, we need a bifunctor $\hom(A,B) × \hom(X,A) \to \hom(X,B)$.  But
we already have such a bifunctor: the composition bifunctor of
$\bicat{C}$!
...
associator and left unitor in $\bicat{C}$.
