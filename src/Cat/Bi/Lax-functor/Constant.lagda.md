...
A **constant pseudofunctor** from $\bicat{B}$ to $\bicat{C}$ sends every
object of $\bicat{B}$ to a single object $c : \bicat{C}$, every 1-cell
of $\bicat{C}$ to the identity morphism on $c$, and every 2-cell of
$\bicat{C}$ to the identity morphism on that identity morphism.
...
Furthermore, given two bicategories $\bicat{B}$ and $\bicat{C}$, the
