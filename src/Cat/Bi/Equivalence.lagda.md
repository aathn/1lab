...
In words, a 1-cell $f : A \to B$ in $\bf{C}$ is an equivalence if there
is a 1-cell $f\inv : B \to A$ in the opposite direction such that
$f\inv$ is an inverse to $f$ up to chosen 2-cell isomorphisms satisfying
the triangle equalities of an [adjunction in] $\bf{C}$.
...
If $F$ and $G$ are lax functors $\bf{B} \to \bf{C}$, we define a
